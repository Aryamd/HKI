                ...
                <div class="card-body">
                    <form class="needs-validation" enctype="multipart/form-data" action="{{ route('merek.update', $id) }}" method="post" novalidate>
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                <div class="position-relative mb-3">
                                    <label for="file_sertifikat" class="form-label @if (\App\Helpers\Utils::isNullOrEmpty($merek->file_sertifikat)) text-danger @endif">Sertifikat Merek</label>
                                    <div class="row">
                                        <div class="col-6">
                                            <input type="file" class="form-control" id="file_sertifikat" name="file_sertifikat" />
                                        </div>
                                        <div class="col-6">
                                            @if (\App\Helpers\Utils::isNullOrEmpty($merek->file_sertifikat))
                                            <label class="form-label mt-1 text-danger">Sertifikat belum diupload</label>
                                            @else
                                            <label class="form-label mt-1">{{ $merek->file_sertifikat }}</label>
                                            @endif
                                        </div>
                                    </div>
                                </div>
